...
The doctor grows a companion `ca-rotation` check (separate from the `ca` *expiry* check). It reads the most recent entry in `rotation-history.jsonl` — or, if that file doesn't exist yet, falls back to the cert's own `notBefore` date — and reports:

- **pass** — last rotation within 365 days.
- **warn** — last rotation between one year and ~nine years ago, with the fix-it hint to run `hermes egress rotate-ca`.
- Hard expiry stays the `ca` check's responsibility; `ca-rotation` never duplicates it.

Verifying that already-running sandboxes still trust the new CA after a rotation (a `verify-trust` subcommand) is a planned follow-up and is **not** part of this release.
