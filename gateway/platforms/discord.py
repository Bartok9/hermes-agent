...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _handle_message(self, message: DiscordMessage) -> None:
        ...
        if not is_thread and not isinstance(message.channel, discord.DMChannel):
            ...
            # Free-response channels (and voice-linked channels) bypass the
            # @mention gate, so they must also bypass auto-threading — they
            # are meant for lightweight inline chat, not a thread per message.
            if (
                auto_thread
                and not skip_thread
