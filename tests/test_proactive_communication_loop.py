...
# Recent timestamp so seeded messages fall inside the loop's history window
# (DEFAULT_HISTORY_WINDOW_HOURS = 72). Must be relative to "now" rather than a
# fixed epoch, otherwise these tests silently bit-rot once wall-clock time
# moves past the hardcoded value and history loads empty.
_RECENT_TS = time.time() - 3600
...
def test_graph_traversal_failure_stays_silent():
    ...
    db = MagicMock()
    db.get_messages.return_value = [{"role": "user", "content": "hello", "timestamp": _RECENT_TS}]
    db.get_meta.return_value = None
    cfg = MagicMock()
    cfg.get.return_value = "conservative"
...
def test_temporal_bridge_high_score_sends():
    ...
    db = MagicMock()
    db.get_messages.return_value = [
        {"role": "user", "content": "working on anomaly detection in time-series today", "timestamp": _RECENT_TS},
    ]
    db.get_meta.return_value = None
    cfg = MagicMock()
    cfg.get.side_effect = lambda k, d=None: {
...
def test_model_veto_respected():
    ...
    db = MagicMock()
    db.get_messages.return_value = [{"role": "user", "content": "something", "timestamp": _RECENT_TS}]
    db.get_meta.return_value = None
    cfg = MagicMock()
    cfg.get.side_effect = lambda k, d=None: {
