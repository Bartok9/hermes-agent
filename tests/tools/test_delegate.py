...
class TestDelegationCredentialResolution(unittest.TestCase):
    ...
    def test_provider_resolves_full_credentials(self, mock_resolve):
        ...
        # target_model is forwarded so api_mode is derived from the model the
        # child is switching TO (resolve_runtime_provider feature, commit be87a96).
    ...
    def test_provider_resolution_uses_runtime_model_when_config_model_missing(self, mock_resolve):
        ...
        # Empty config model → configured_model is None → target_model=None.
        mock_resolve.assert_called_once_with(
...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        def slow_run(**kwargs):
            # Long enough that, under the idle threshold (5 cycles =
            # 0.25s), the buggy idle-branch would have stopped firing well
            # before the run ends. The generous runtime (1.5s vs 0.05s
            # interval => ~30 nominal cycles) gives wide margin against
            # thread-scheduling starvation on a loaded CI box, where the
            # heartbeat thread may only get a fraction of its slots.
            time.sleep(1.5)
            return {"final_response": "done", "completed": True, "api_calls": 1}

        child.run_conversation.side_effect = slow_run

        # Pin BOTH the interval and the idle ceiling so the test proves the
        # in-tool branch takes effect deterministically: with interval=0.05s
        # and _HEARTBEAT_STALE_CYCLES_IDLE=5, the OLD (buggy) behavior would
        # trip the idle threshold at ~0.25s and stop firing. Because the
        # child is inside a tool, the in-tool branch keeps the heartbeat
        # alive for the whole 0.7s run.
        with patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05), \
        ...
        # With the idle threshold (5 cycles = 0.25s), touch_calls would cap
        # at ~5. With the in-tool threshold the heartbeat keeps firing for
        # the full 1.5s run (~30 nominal touches), so > 6 robustly proves
        # the in-tool branch is taking effect even under heavy timing
        # jitter / thread starvation.
        self.assertGreater(
            len(touch_calls), 6,
            f"Heartbeat stopped too early while child was inside a tool; "
            f"got {len(touch_calls)} touches over 1.5s at 0.05s interval",
        )
    ...
    def test_heartbeat_still_trips_idle_stale_when_no_tool(self):
        ...
        # Pin the idle threshold so the test exercises the BEHAVIOR (idle
        # stale fires) independently of production's tuned constant
        # (_HEARTBEAT_STALE_CYCLES_IDLE, which has since risen to 15). With
        # idle threshold=5 and interval=0.05s the heartbeat trips at ~0.25s,
        # well before the 0.6s run, so it must stop touching the parent.
        with patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05), \
             patch("tools.delegate_tool._HEARTBEAT_STALE_CYCLES_IDLE", 5):
            _run_single_child(
                task_index=0,
